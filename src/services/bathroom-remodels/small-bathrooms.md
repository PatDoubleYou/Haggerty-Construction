---
layout: 'service.html'
title: 'Small Bathroom Remodels'
subtitle: 'Small space. Big improvement.'
tagTitle: 'Small Bathroom Remodel in St. George, Utah'
description: 'Small bathroom remodeling in St. George, Utah. Smart layouts, space-saving solutions, custom tile. Haggerty Construction — free estimates.'
metaTitle: 'Small Bathroom Remodel St. George | Haggerty Construction'
heroImage: '/images/bathroom-door-renovation-850w.webp'
heroImageAlt: 'Small bathroom remodel by Haggerty Construction in St. George'
permalink: '/services/bathroom-remodels/small-bathrooms/'
serviceType: 'Small Bathroom Remodeling'
parentService:
  title: 'Bathroom Renovations'
  url: '/services/bathroom-remodels/'
eleventyNavigation:
    key: Small Bathroom Remodels
    parent: Bathroom Renovations
    order: 330
capabilities:
  - icon: 'bi-arrows-angle-expand'
    title: 'Space-Smart Layouts'
    text: 'Strategic fixture placement, pocket doors, and corner solutions that make every square foot count.'
  - icon: 'bi-gem'
    title: 'Glass Enclosures'
    text: 'Frameless glass showers open the sightlines and make a small bathroom feel twice as large.'
  - icon: 'bi-grid-3x3'
    title: 'Large-Format Tile'
    text: 'Fewer grout lines, cleaner look. Large tiles reduce visual clutter and make small spaces feel more open.'
  - icon: 'bi-symmetry-horizontal'
    title: 'Floating Vanities'
    text: 'Wall-mounted vanities free up floor space and create a modern, airy look.'
  - icon: 'bi-lightbulb'
    title: 'Layered Lighting'
    text: 'Recessed cans, sconces, and backlit mirrors — proper lighting makes small rooms feel larger.'
  - icon: 'bi-box-seam'
    title: 'Built-In Storage'
    text: 'Recessed medicine cabinets, shower niches, and vertical shelving designed into the walls.'
process:
  - title: 'Free Consultation'
    text: 'We measure your bathroom and talk about what matters most to you in the space.'
  - title: 'Design & Estimate'
    text: 'A plan that maximizes your space, with honest pricing and a clear timeline.'
  - title: 'Build'
    text: 'Demo, plumbing, tile, fixtures, paint — every trade coordinated by our team.'
  - title: 'Final Walkthrough'
    text: 'We make sure every detail is right before we call it done.'
faqs:
  - question: 'Can you really make my small bathroom feel bigger?'
    answer: 'Yes. Glass shower enclosures instead of curtains, large-format tile, floating vanities, recessed storage, and proper lighting all work together to make a small bathroom feel significantly more open. We do this regularly.'
  - question: 'How much does a small bathroom remodel cost?'
    answer: 'A guest bath or half-bath refresh typically ranges from $6,000 to $15,000 depending on materials and scope. A full small bathroom remodel with new tile, vanity, and fixtures runs $12,000 to $22,000. We give you a detailed estimate upfront.'
  - question: 'Should I keep the tub in a small bathroom?'
    answer: 'It depends. If it is your only bathroom with a tub, keeping it may be wise for resale. But in many cases, replacing the tub with a walk-in shower opens up the room dramatically. We will give you our honest recommendation.'
  - question: 'How long does a small bathroom remodel take?'
    answer: 'Most small bathroom remodels take 1-3 weeks depending on the scope. A simple vanity and fixture swap might be done in a week. A full gut with new tile and layout changes takes 2-3 weeks.'
testimonial:
  text: 'Our interaction with the Haggerty brothers has been so positive! They are very professional, prompt, clear, and polite in their communication and I recommend getting in touch with them.'
  name: 'Taylor Sanchez'
relatedServices:
  - title: 'Bathroom Renovations'
    url: '/services/bathroom-remodels/'
    description: 'Full bathroom remodels for any size space.'
  - title: 'Shower Remodels'
    url: '/services/bathroom-remodels/shower-remodels/'
    description: 'Custom tile showers and glass enclosures.'
  - title: 'Tub-to-Shower Conversions'
    url: '/services/bathroom-remodels/tub-to-shower/'
    description: 'Free up space by replacing the tub with a walk-in shower.'
---

A small bathroom does not have to feel small. The problem usually is not the square footage — it is that the space was designed without thought. A bulky vanity eating up floor space. A shower curtain cutting the room in half visually. Dark tile, no storage, and a single overhead light making everything feel cramped.

We remodel small bathrooms all the time, and the results consistently surprise homeowners. The same footprint can feel completely different when the layout, materials, and lighting are chosen intentionally.

## How we approach small bathroom remodels

**We design around the constraints.** Every small bathroom is a puzzle. The fixture placement, the door swing, the storage — it all has to work together in a tight space. We plan every inch before we demo anything.

**We use materials that open up the room.** Large-format tile with minimal grout lines. Frameless glass instead of a shower curtain. Floating vanities that reveal the floor. Recessed storage built into the walls instead of freestanding shelves. These choices make a measurable difference.

**We do not cut corners just because it is small.** A small bathroom still gets proper waterproofing, level tile, sealed grout, and tested plumbing. The size of the room does not change our standards.

## The guest bath problem

A lot of our small bathroom remodels are guest baths or hall baths that have been ignored for years. Dated tile, a vanity from the early 2000s, and fixtures that look tired. These bathrooms see guests, visiting family, and everyday use — they deserve to look as good as the rest of your home.

**Serving St. George, Washington, Hurricane, and all of Washington County.** Ready to make your small bathroom work harder? Call us or fill out the form below.
